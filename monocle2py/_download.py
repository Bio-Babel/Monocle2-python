...
def resolve_data_path(filename: str) -> Path:
    """Resolve a registered data asset to a local file path.

    Resolution order:

    1. ``~/.cache/<CACHE_DIR_NAME>/<filename>``  — previously downloaded
    2. Download from registry URL → save to cache, verify SHA-256
    ...
    if filename not in REGISTRY:
        raise FileNotFoundError(
            f"'{filename}' is not registered. Known assets: "
            f"{sorted(REGISTRY)}"
        )
